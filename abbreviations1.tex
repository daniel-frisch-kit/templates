...
\def\Eq#1{(\ref{#1})}
\def\Chap#1{Chap.~\ref{#1}}
\def\Sec#1{Sec.~\ref{#1}}
\def\SubSec#1{Subsec.~\ref{#1}}
\def\Ex#1{Example~\ref{#1}} 
\def\Fig#1{Fig.~\ref{#1}}
\def\Tab#1{Table~\ref{#1}}
\def\Alg#1{Alg.~\ref{#1}}
\def\Line#1{line~\ref{#1}}
...
% Figure with [!htbp]
% Usage: \Figure{filename}{caption}{label}{width}
\newcommand{\Figure}[4]{%
	%
	\begin{figure*}[!htbp]
		%
		\begin{center}
			\includegraphics[width=#4\linewidth]{Figures/#1}
		\end{center}
		%
		\caption{#2}
		%
